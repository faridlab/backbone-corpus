//! Integrity probes — the KB invariants: only PUBLISHED articles are served (a draft must never leak) and
//! an archived article is withdrawn; the deflection read is target-scoped.
...
async fn cip1_draft_never_served() {
    ...
    let draft = svc
        .create_article(NewArticle {
            category_id: None,
            title: "WIP — do not show".into(),
            body: "unreviewed".into(),
        })
        .await
        .unwrap();
    svc.link_article(draft, link("support", "issue", issue))
        .await
        .unwrap();
    ...
    assert!(
        served.is_empty(),
        "a draft article must never be served on the deflection read"
    );

    // Once published, it IS served.
    svc.publish_article(draft).await.unwrap();
    assert_eq!(
        svc.suggest_for_target("support", issue)
            .await
            .unwrap()
            .len(),
        1
...
async fn cip2_archived_withdrawn() {
    ...
    let art = svc
        .create_article(NewArticle {
            category_id: None,
            title: "Old answer".into(),
            body: "...".into(),
        })
        .await
        .unwrap();
    svc.link_article(art, link("support", "issue", issue))
        .await
        .unwrap();
...
// CIP-3 — the deflection read is target-scoped: another target's articles are not served.
#[tokio::test]
async fn cip3_scoped_to_target() {
    let pool = pool().await;
    let svc = CorpusWriteService::new(pool.clone());
    ...
    let art = svc
        .create_article(NewArticle {
            category_id: None,
            title: "For issue A".into(),
            body: "...".into(),
        })
        .await
        .unwrap();
    svc.link_article(art, link("support", "issue", issue_a))
        .await
        .unwrap();
    svc.publish_article(art).await.unwrap();

    assert_eq!(
        svc.suggest_for_target("support", issue_a)
            .await
            .unwrap()
            .len(),
        1
    ...
    assert!(
        svc.suggest_for_target("support", issue_b)
            .await
            .unwrap()
            .is_empty(),
        "not served for a different target"
    );
}
