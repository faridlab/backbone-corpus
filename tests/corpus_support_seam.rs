...
async fn cseam1_real_issue_gets_suggested_articles() {
    ...
    let issue = support
        .raise_issue(
            NewIssue {
                ...
                subject: "Card declined at checkout".into(),
                description: None,
                priority: "high".into(),
                sla_id: Some(sla),
            },
    ...
    let art = corpus
        .create_article(NewArticle {
            category_id: None,
            title: "Why was my card declined?".into(),
            body: "Common reasons and fixes...".into(),
        })
        .await
        .unwrap();
    corpus
        .link_article(
            ...
            LinkTarget {
                ...
                target_id: issue,
                category_key: Some("payments".into()),
            },
        )
        .await
        .unwrap();
    ...
    let served = corpus.suggest_for_target("support", issue).await.unwrap();
    assert_eq!(
        served.len(),
        1,
        "the real issue surfaces its linked published article"
    );
