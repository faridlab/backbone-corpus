...
metaphor make entity Vendor --module corpus
...
metaphor schema schema validate corpus

# 3. Generate all artifacts (entity, DTOs, repo, service, handler, routes).
metaphor schema schema generate corpus --target all --force

# 4. Generate the migration for the new entity.
metaphor migration generate Vendor corpus
...
> `corpus` is your module name (auto-detected from the current directory when omitted).
...
2. `metaphor schema schema validate corpus`.
3. Generate a migration for the change:
   `metaphor migration generate <Entity> corpus` (or, for a schema-diff-driven migration against
   a live DB, `metaphor schema schema migration corpus --database-url …`).
4. Regenerate code: `metaphor schema schema generate corpus --target all --force`.
