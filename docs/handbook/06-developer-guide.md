...
metaphor migration generate Invoice corpus
metaphor migration run
...
(`corpus` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed corpus          # run Rust seeders
metaphor migration generate-seeds corpus  # emit SQL seed files
```
