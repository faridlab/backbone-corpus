...
/// Create stateless routes for the Corpus module.
///
...
///
/// ```ignore
/// let module = CorpusModule::builder().with_database(pool).build()?;
/// let routes = create_stateless_routes(&module);
/// let app = Router::new().merge(routes);
/// ```
pub fn create_stateless_routes(module: &crate::CorpusModule) -> Router<()> {
    Router::new()
        .merge(create_article_category_routes(module.article_category_service.clone()))
        .merge(create_article_routes(module.article_service.clone()))
        .merge(create_article_link_routes(module.article_link_service.clone()))
        .merge(create_article_feedback_routes(module.article_feedback_service.clone()))
}

/// Read-only routes for the Corpus module — every entity mounted READ-ONLY (the guarded base).
///
...
/// service's invariants. Extend it: `create_readonly_corpus_routes(m).merge(my_validated_writes)`.
pub fn create_readonly_corpus_routes(module: &crate::CorpusModule) -> Router<()> {
    Router::new()
        .merge(create_article_category_read_routes(module.article_category_service.clone()))
        .merge(create_article_read_routes(module.article_service.clone()))
        .merge(create_article_link_read_routes(module.article_link_service.clone()))
        .merge(create_article_feedback_read_routes(module.article_feedback_service.clone()))
}

/// Get all routes (stateless) for the Corpus module.
///
/// This is a convenience function that wraps routes with API versioning.
pub fn get_routes(module: &crate::CorpusModule) -> Router<()> {
    Router::new()
...
/// Create stateful routes for the Corpus module.
///
...
pub fn create_combined_routes(module: &crate::CorpusModule) -> Router<AppState> {
...
/// Get all routes (stateful) for the Corpus module.
///
/// This is a convenience function that wraps routes with API versioning.
pub fn get_routes_with_state(module: &crate::CorpusModule) -> Router<AppState> {
    Router::new()
