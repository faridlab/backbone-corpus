//! Application service for ArticleFeedback entities
//!
//! Generated by metaphor-schema. Do not edit manually.
//!
//! Type alias over `GenericCrudService` — all CRUD logic lives in backbone-core.
//! For custom behaviour, create `article_feedback_service_custom.rs` (preserved on regen).

use backbone_core::GenericCrudService;

use crate::presentation::dto::{CreateArticleFeedbackDto, UpdateArticleFeedbackDto};
use crate::domain::entity::ArticleFeedback;
use crate::infrastructure::persistence::ArticleFeedbackRepository;

/// Application service for ArticleFeedback entities.
///
/// All 12 standard CRUD operations (list, create, get, update, patch,
/// soft_delete, restore, empty_trash, bulk_create, upsert, find_by_id,
/// list_deleted) are provided by `GenericCrudService`.
///
/// Custom extensions go in `article_feedback_service_custom.rs`:
///
/// ```rust,ignore
/// pub struct ArticleFeedbackServiceCustom {
///     inner: Arc<ArticleFeedbackService>,
///     // add domain-specific dependencies here
/// }
/// ```
pub type ArticleFeedbackService = GenericCrudService<
    ArticleFeedback,
    CreateArticleFeedbackDto,
    UpdateArticleFeedbackDto,
    ArticleFeedbackRepository,
>;

// <<< CUSTOM
// END CUSTOM
