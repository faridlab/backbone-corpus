//! Application service for Article entities
//!
//! Generated by metaphor-schema. Do not edit manually.
//!
//! Type alias over `GenericCrudService` — all CRUD logic lives in backbone-core.
//! For custom behaviour, create `article_service_custom.rs` (preserved on regen).

use backbone_core::GenericCrudService;

use crate::presentation::dto::{CreateArticleDto, UpdateArticleDto};
use crate::domain::entity::Article;
use crate::infrastructure::persistence::ArticleRepository;

/// Application service for Article entities.
///
/// All 12 standard CRUD operations (list, create, get, update, patch,
/// soft_delete, restore, empty_trash, bulk_create, upsert, find_by_id,
/// list_deleted) are provided by `GenericCrudService`.
///
/// Custom extensions go in `article_service_custom.rs`:
///
/// ```rust,ignore
/// pub struct ArticleServiceCustom {
///     inner: Arc<ArticleService>,
///     // add domain-specific dependencies here
/// }
/// ```
pub type ArticleService = GenericCrudService<
    Article,
    CreateArticleDto,
    UpdateArticleDto,
    ArticleRepository,
>;

// <<< CUSTOM
// END CUSTOM
